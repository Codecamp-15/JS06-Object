...
const product = {};
const calPrice = (name) => name.cost - name.cost*(name.discount/100);
let name;
let quantity;
let cost;
let discount;
...
do {
    ...
    cost = prompt('Enter cost of product.');
    discount = prompt('Enter discount(0-100%).');
    if(+discount==0||discount=='undefined'){
        product[name] ={quantity,cost,};
    }else{
        product[name] ={quantity,cost,discount};
        finalPrice=calPrice(product[name])
        product[name] ={quantity,cost,discount,finalPrice};
    }
